...
class SupportVectorMachines:
    ...
    def support_vector_machines(self, training_set_a, training_set_b, testing_set_a, testing_set_b):

        print(" \n > Applying RBF Support Vector Machines By Using Sklearn (It may takes several minutes...)")

        # Train the data
        start = time.time()
        cls = self.train_svm_dataset_sklearn(training_set_a, training_set_b, "rbf")
        end = time.time()
        ...
        true_positive, false_negative, true_negative, false_positive, test_pred_proba, test_labels = \
            self.test_svm_dataset_sklearn(testing_set_a, testing_set_b, cls)
        end = time.time()
    ...
    def SVM_cross_validation(self, training_set_a, training_set_b):
        lowest_error = 100
        best_index = 0
        array_index = []
        array_error = []
        ...
        for i in range(1,5):
            ...
            error, true_positive, true_negative, false_negative, false_positive, test_pred_proba, test_labels \
                = self.support_vector_machines(fold_train_a, fold_train_b, fold_validation_a, fold_validation_b)
        ...
        df.to_excel('SVM Cross Validation Partition Data.xlsx', sheet_name='sheet1', index=False) 
